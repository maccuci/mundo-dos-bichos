    ...
    return (
        <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow">
            <li key={id} className="flex flex-col items-center pb-10">
                <p className="mt-1 mb-1 text-xl font-medium text-black">Informações de {petName} ({id})</p>
                <p className="text-sm text-gray-500">Serviço: {service}</p>
                <p className="text-sm text-gray-500">Preço: R$ {price}</p>
                <p className="text-sm text-gray-500">Data: {new Date(Date.parse(date)).toLocaleString()}</p>
